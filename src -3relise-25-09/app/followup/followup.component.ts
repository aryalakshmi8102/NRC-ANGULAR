...
okay: any;
  ...
  this.retrieved = JSON.parse (localStorage.getItem('childData1'));
   this.okay = this.retrieved;
  
   console.log('dfghj', this.okay)
  ...
  onSubmitFollowup(FollowupForm:any){
    ...
    this.data.SaveFollowup(FollowupForm,this.caseId,this.followupsdvalue,this.currentUser.nrcUId)
    .subscribe(
        response => {
          if(response >= 0){
            console.log("Success");
            alert(' New follow-up   saved successfully');
           // this.router.navigateByUrl('/');   
          location.reload();        
          } else {
            alert(" New follow-up  Not Saved");
          }
